...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const users = yield Promise.all([
            prisma.user.create({
                data: {
                    ...
                    profileVisibility: prisma_1.ProfileVisibility.PUBLIC,
                },
            }),
            prisma.user.create({
                data: {
        ...
        const events = yield Promise.all([
            prisma.event.create({
                data: {
                    ...
                    privacyLevel: prisma_1.PrivacyLevel.PUBLIC,
                    hostId: users[0].id,
                    weather: {
                        temperature: 72,
                        condition: 'Clear',
        ...
        for (let i = 1; i < users.length; i++) {
            attendees.push(prisma.attendee.create({
                data: {
                    userId: users[i].id,
                    eventId: events[0].id,
                    rsvp: i === 1 ? prisma_1.RSVP.YES : i === 2 ? prisma_1.RSVP.MAYBE : prisma_1.RSVP.YES,
        ...
        yield Promise.all(attendees);
        console.log(`✅ Created ${attendees.length} attendee records`);
        // Create system settings
        console.log('⚙️ Creating system settings...');
        yield prisma.systemSettings.createMany({
            data: [
                {
                    key: 'app_version',
                    value: '1.0.0',
